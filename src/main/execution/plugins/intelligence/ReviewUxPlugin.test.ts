...
vi.mock('electron', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'pawos-review-ux-test-'));
...
import { reviewUxPlugin } from './ReviewUxPlugin';
...
const PAGES: Record<string, string> = {
  '/': '<html lang="en"><head><title>Home</title></head><body><nav><a href="/about">About</a></nav><h1>Home</h1></body></html>',
...
describe('ReviewUxPlugin', () => {
  ...
  it('requires confirmation every time — never remembers a prior approval', async () => {
    ...
    const first = await reviewUxPlugin.execute({ type: 'reviewUx', url: server.origin });
    expect(first.ok).toBe(false);
    if (first.ok) return;
    expect(first.reason).toBe('requires-confirmation');

    const second = await reviewUxPlugin.execute({ type: 'reviewUx', url: server.origin });
    expect(second.ok).toBe(false);
    if (second.ok) return;
    expect(second.reason).toBe('requires-confirmation');
  });

  it('reviews multiple real pages once confirmed, persists the report, and discloses the visual-layout gap', async () => {
    const server = await startServer(handler);
    close = server.close;

    const result = await reviewUxPlugin.execute({ type: 'reviewUx', url: server.origin, confirmed: true, maxPages: 10, maxDepth: 2 });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    const report = result.data as IntelligenceReport<UxReportFields>;
    expect(report.engineId).toBe('ux');
    ...
    const persisted = findLatestIntelligenceReport('ux', server.origin);
  ...
  it('clamps maxPages/maxDepth to hard maximums regardless of what was requested', async () => {
    ...
    const result = await reviewUxPlugin.execute({
      type: 'reviewUx',
      url: server.origin,
      confirmed: true,
      maxPages: 999,
  ...
  it('reports a clear, honest failure when the site is unreachable', async () => {
    const result = await reviewUxPlugin.execute({ type: 'reviewUx', url: 'http://127.0.0.1:1', confirmed: true });
    expect(result.ok).toBe(false);
  });

  it('surfaces an invalid-URL requirement instead of executing blind', () => {
    const requirements = reviewUxPlugin.requirements({ type: 'reviewUx', url: 'not-a-url' });
  ...
  it('describes the pending confirmation with the actual (clamped) scope', () => {
    const description = reviewUxPlugin.describeDone(
      { type: 'reviewUx', url: 'https://example.com', maxPages: 999 },

 ...
 *
 * Note: This function does NOT round because the input workPc is already rounded
 * by providerCostToWorkPc() to 4 decimals. Applying rounding again here would
 * introduce artificial drift that breaks margin verification (verifyMargin).
 * Division by 100 is exact, so no rounding is needed.
 */
export function workPcToCustomerCharge(workPc: number): number {
  return workPc / WORK_PC_PER_CUSTOMER_DOLLAR;
}

/**
 * Convert normalized compute back to provider cost (USD).
 *
 * Inverse of the normalization formula: provider_cost = normalized_compute / 1000
 */
export function normalizedComputeToProviderCost(normalizedCompute: number): number {
  return normalizedCompute / 1000;
}

/**
 * Convert normalized compute to customer Work PC (via provider cost).
 *
 * Complete path: normalized compute → provider cost → customer charge → Work PC
 */
export function normalizedComputeToWorkPc(normalizedCompute: number): number {
  const providerCostUsd = normalizedComputeToProviderCost(normalizedCompute);
  return providerCostToWorkPc(providerCostUsd);
}

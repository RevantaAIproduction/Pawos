...
proc.stderr.on('data', (d) => {
  ...
  if (txt.includes('error') || txt.includes('Error')) {
    console.log('[STDERR]', txt.substring(0, 200));
  }
});
      ...
      http.get('http://localhost:9222/json', (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            console.log('Failed to parse tabs:', e.message);
            resolve([]);
          }
        });
      }).on('error', (err) => {
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (msg.id === 1 && msg.result) {
          ...
          const screenshotPath = 'C:\\Users\\APPLE\\Downloads\\PawOS\\comms-panel-latest.png';
